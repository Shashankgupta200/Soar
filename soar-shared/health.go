...
func RunOpsAppHealthCheck(apiKey string, orgId string) (AppHealth, error) {
	...
	if os.Getenv("SOAR_CLOUDRUN_URL") != "" {
	...
	//	newOpenapiString = strings.Replace(newOpenapiString, `"title":"Soar"`, `"title":"Soar-Copy"`, 1)
	//	newOpenapiString = strings.Replace(newOpenapiString, `"version":"1.0"`, `"version":"2.0"`, 1)
	//	newOpenapiString = strings.Replace(newOpenapiString, `"tags":[{"name":"SOAR"},{"name":"Automation"},{"name":"Soar"}]`, `"tags":[]`, 1)
...
func deleteJunkOpsWorkflow(ctx context.Context, workflowHealth WorkflowHealth) error {
	...
	if err != nil {
		//log.Printf("[DEBUG] Failed finding any workflow named SOAR_INTERNAL_OPS_WORKFLOW: %s. Is the health API initialized?", err)
		return err
	}

	if len(workflows) == 0 {
		//log.Printf("[DEBUG] Couldn't find any workflow named SOAR_INTERNAL_OPS_WORKFLOW")
		return errors.New("Failed finding workflow named SOAR_INTERNAL_OPS_WORKFLOW")
	}

	//log.Printf("[DEBUG] Found %d workflows named SOAR_INTERNAL_OPS_WORKFLOW: ", len(workflows))
...
func RunOpsHealthCheck(resp http.ResponseWriter, request *http.Request) {
	...
	if os.Getenv("SOAR_HEALTHCHECK_DISABLED") == "true" {
		resp.WriteHeader(200)
		resp.Write([]byte(`{"success": false, "reason": "Healthcheck disabled (not default). Set SOAR_HEALTHCHECK_DISABLED=false to re-enable it."}`))
		return
	}

	// Allows overwrites if they exist
	apiKey := os.Getenv("SOAR_OPS_DASHBOARD_APIKEY")
	orgId := os.Getenv("SOAR_OPS_DASHBOARD_ORG")
	...
	if project.Environment != "onprem" {
		...
		if err != nil {
			...
			if err != nil {
				log.Printf("[ERROR] Failed marshalling platform health data: %s", err)
				resp.WriteHeader(500)
				resp.Write([]byte(`{"success": false, "reason": "Failed JSON parsing platform health. Contact support@soarr.io"}`))
				return
			}
	...
	if project.Environment == "onprem" && userInfo.Role != "admin" {
		...
		return
	} else if project.Environment == "Cloud" && (userInfo.ApiKey != os.Getenv("SOAR_OPS_DASHBOARD_APIKEY") || userInfo.SupportAccess) {
		resp.WriteHeader(401)
		resp.Write([]byte(`{"success": false, "reason": "Only admins can run health check!"}`))
		return
	}
	...
	//log.Printf("[DEBUG] Is user api key same as ops dashboard api key? %t", userInfo.ApiKey == os.Getenv("SOAR_OPS_DASHBOARD_APIKEY"))
...
func fixOpensearch() error {
	...
	opensearchUrl := os.Getenv("SOAR_OPENSEARCH_URL")
...
func fixHealthSubflowParameters(ctx context.Context, workflow *Workflow) (Workflow, error) {
	...
	for i := range workflow.Triggers {
		if workflow.Triggers[i].AppName != "Soar Workflow" {
			continue
		}
...
func RunOpsWorkflow(apiKey string, orgId string, cloudRunUrl string) (WorkflowHealth, error) {
	...
	workflowHealth := WorkflowHealth{
		Create:      false,
		BackendVersion: os.Getenv("SOAR_BACKEND_VERSION"),
		Run:         false,
		RunFinished: false,
		ExecutionTook: 0,
		Delete:      false,
	...
	for workflowHealth.RunFinished == false && workflowHealth.Run == true {
		...
		reqBody := map[string]string{"execution_id": execution.ExecutionId, "authorization": os.Getenv("SOAR_OPS_DASHBOARD_APIKEY")}
...
func RunOpsAppUpload(apiKey string, orgId string) (AppHealth, error){
	...
	go func() {
		...
		part, err := writer.CreateFormFile("soar_file", "app.zip")
		if err != nil {
	...
	executeBody.AppName = "Soar Tools Copy"
...
func InitOpsWorkflow(apiKey string, OrgId string) (string, error) {
	...
	if project.Environment == "cloud" {
		// url := "https://soarr.io/api/v1/workflows/602c7cf5-500e-4bd1-8a97-aa5bc8a554e6"
		// url := "https://soarr.io/api/v1/workflows/7b729319-b395-4ba3-b497-d8246da67b1c"
		// url := "https://soarr.io/api/v1/workflows/412256ca-ce62-4d20-9e55-1491548349e1"
		url := "https://soarr.io/api/v1/workflows/ae89a788-a26b-4866-8a0b-ce0b31d354ea"
		req, err := http.NewRequest("GET", url, nil)
		if err != nil {
	...
	// make a POST request to https://soarr.io/api/v1/workflows
	baseUrl := os.Getenv("SOAR_CLOUDRUN_URL")
	if len(baseUrl) == 0 {
		log.Printf("[DEBUG] Base url not set. Setting to default")
		baseUrl = "https://soarr.io"
	...
	jsonData := `{"name":"SOAR_INTERNAL_OPS_WORKFLOW","description":"demo","hidden":true,"blogpost":"","status":"test","default_return_value":"","usecase_ids":[]}`
...
func GetStaticWorkflowHealth(ctx context.Context, workflow Workflow) (Workflow, []string, error) {
	...
	allNames := []string{}
	for _, action := range workflow.Actions {
		if action.AppID == "integration" || action.AppID == "soar_agent" {
	...
	for _, action := range workflow.Actions {
		...
		if builtin {
			...
			if curapp.ID == "" && action.AppID != "integration" && action.AppID != "soar_agent" {
			...
			if curapp.ID != action.AppID && curapp.Name != action.AppName {
				...
				for _, param := range curappaction.Parameters {
					...
					if param.Value == "" && param.Variant == "STATIC_VALUE" && param.Required == true {
						...
						if ((strings.ToLower(action.AppName) == "http" && param.Name == "body") || (strings.ToLower(action.Name) == "send_sms_soar" || strings.ToLower(action.Name) == "send_email_soar") && param.Name == "apikey") || (action.Name == "repeat_back_to_me") || (action.Name == "filter_list" && param.Name == "field") || action.Name == "custom_action" {
							// Do nothing
						} else {
...
func HandleRerunExecutions(resp http.ResponseWriter, request *http.Request) {
	...
	if strings.ToLower(os.Getenv("SOAR_DISABLE_RERUN_AND_ABORT")) == "true" {
		//log.Printf("[AUDIT] Rerunning is disabled by the SOAR_DISABLE_RERUN_AND_ABORT argument. Stopping.")
		resp.WriteHeader(409)
		resp.Write([]byte(fmt.Sprintf(`{"success": false, "reason": "SOAR_DISABLE_RERUN_AND_ABORT is active. Won't rerun executions."}`)))
		return
	}
...
func HandleStopExecutions(resp http.ResponseWriter, request *http.Request) {
	...
	if strings.ToLower(os.Getenv("SOAR_DISABLE_RERUN_AND_ABORT")) == "true" {
		if ok && deleteAll[0] == "true" {
			...
			//log.Printf("[AUDIT] Rerunning is disabled by the SOAR_DISABLE_RERUN_AND_ABORT argument. Stopping. (abort)")
			resp.WriteHeader(409)
			resp.Write([]byte(fmt.Sprintf(`{"success": false, "reason": "SOAR_DISABLE_RERUN_AND_ABORT is active. Won't rerun executions (abort)"}`)))
			return
		}
	}
