...
/*
	Handles files for Soar. Uses ID's to reference everything
*/
...
var basepath = os.Getenv("SOAR_FILE_LOCATION")
var orgFileBucket = "soar_org_files"
var maxFileSize = 10000000 // raw 10mb max filesize on cloud

func init() {
	if len(os.Getenv("SOAR_ORG_BUCKET")) > 0 {
		orgFileBucket = os.Getenv("SOAR_ORG_BUCKET")
	} else {
...
func HandleGetFileNamespace(resp http.ResponseWriter, request *http.Request) {
	...
	// For now it's using Soar's repo for standards, but this could
...
func HandleEditFile(resp http.ResponseWriter, request *http.Request) {
	...
	if err != nil {
		...
		resp.WriteHeader(500)
		resp.Write([]byte(`{"success": false, "reason": "Failed file upload in Soar"}`))
		return
	}
	...
	if file.Namespace == "sigma" {
		...
		err = SetDetectionOrborusRequest(ctx, user.ActiveOrg.Id, execType, file.Filename, "SIGMA", "SOAR_DISCOVER")
		if err != nil {
...
func HandleUploadFile(resp http.ResponseWriter, request *http.Request) {
	...
	parsedFile, _, err := request.FormFile("soar_file")
	if err != nil {
		log.Printf("[ERROR] Failed to upload file: '%s'", err)
		resp.WriteHeader(400)
		resp.Write([]byte(`{"success": false, "reason": "Failed uploading file. Correct usage is: soar_file=@filepath"}`))
		return
	}
...
func HandleCreateFile(resp http.ResponseWriter, request *http.Request) {
	...
	if curfile.WorkflowId != "global" {
		for _, trigger := range workflow.Triggers {
			if trigger.AppName == "Soar Workflow" && trigger.TriggerType == "SUBFLOW" {
