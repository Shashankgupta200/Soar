...
var gceProject = "soarr"
var bucketName = "soarr.appspot.com"
var publicBucket = "soar_public"
...
var appbasefile = "/home/$USER/git/app_sdk/soar_sdk/soar_sdk.py"
...
func addRequirements(filelocation string) {
	...
	filedata := soar.GetAppRequirements() + string(data)
...
func deployFunction(appname, localization, applocation string, environmentVariables map[string]string) error {
	...
	serviceAccountEmail := "soar-apps@soarr.iam.gserviceaccount.com"

	if len(gceProject) > 0 {
		serviceAccountEmail = fmt.Sprintf("soar-apps@%s.iam.gserviceaccount.com", gceProject)
	}
...
func deployAppCloudFunc(appname string, appversion string) {
	...
	if len(os.Getenv("SOAR_GCE_LOCATION")) > 0 {
		locations = []string{os.Getenv("SOAR_GCE_LOCATION")}
	}
...
func loadYaml(fileLocation string) (soar.WorkflowApp, error) {
	action := soar.WorkflowApp{}

	yamlFile, err := ioutil.ReadFile(fileLocation)
	if err != nil {
		log.Printf("[WARNING] yamlFile.Get err: %s", err)
		return soar.WorkflowApp{}, err
	}
...
// Sends a rebuild request to the soar API
...
func main() {
	//addRequirements("generated_apps/soar-tools_1.0.0/requirements.txt")
	if len(os.Args) < 3 {
		log.Printf("[WARNING] Missing arguments. <> are NOT required. Input: go run stitcher.go APIKEY URL <GCEPROJECT> <GCE_REGION> <BUCKETNAME>\n\n\nSample: go run stitcher.go APIKEY https://ca.soarr.io soar-na-northeast1 northamerica-northeast1 soar_org_files_na_northeast1") 
		return
	}

	if len(os.Getenv("SOAR_ORG_BUCKET")) > 0 {
		bucketName = os.Getenv("SOAR_ORG_BUCKET")
	}

	if len(os.Getenv("SOAR_GCEPROJECT")) > 0 {
		gceProject = os.Getenv("SOAR_GCEPROJECT")
	}

	if len(os.Getenv("SOAR_GCEPROJECT_REGION")) > 0 {
	...
	sendRebuildRequest(fmt.Sprintf("frikky/soar:%s_%s", appname, appversion))
}
