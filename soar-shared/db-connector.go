...
import (
	...
	//opensearch "github.com/Shashankgupta200/Soar/opensearch-go"
	opensearch "github.com/opensearch-project/opensearch-go"
	"github.com/opensearch-project/opensearch-go/v2/opensearchapi"
)

var requestCache = cache.New(60*time.Minute, 60*time.Minute)
var memcached = os.Getenv("SOAR_MEMCACHED")
var mc = gomemcache.New(memcached)
var gceProject = os.Getenv("SOAR_GCEPROJECT")
var propagateUrl = os.Getenv("SOAR_PROPAGATE_URL")
var propagateToken = os.Getenv("SOAR_PROPAGATE_TOKEN")

var maxCacheSize = 1020000

// Dumps data from cache to DB for every {dbInterval} action (tried 5, 10, 25)
type SoarStorage struct {
	GceProject    string
...
func GetESIndexPrefix(index string) string {
	prefix := os.Getenv("SOAR_OPENSEARCH_INDEX_PREFIX")
	if len(prefix) > 0 {
...
func GetCache(ctx context.Context, name string) (interface{}, error) {
	...
	if len(memcached) > 0 {
		...
		return "", errors.New(fmt.Sprintf("No cache found in SOAR_MEMCACHED for %s", name))
	}
...
func SetWorkflowExecution(ctx context.Context, workflowExecution WorkflowExecution, dbSave bool) error {
	...
	if err != nil || hostname == "debian" {
		hostname = "soar-backend"
	}

	// FIXME: This right here has caused more problems during dev than anything
	if (os.Getenv("SOAR_SWARM_CONFIG") == "run" || project.Environment == "worker") && !strings.Contains(strings.ToLower(hostname), "backend") {
		if debug {
...
func getExecutionFileValue(ctx context.Context, workflowExecution WorkflowExecution, action ActionResult) (string, error) {
	projectName := os.Getenv("SOAR_GCEPROJECT")
...
func Fixexecution(ctx context.Context, workflowExecution WorkflowExecution) (WorkflowExecution, bool) {
	...
	for actionIndex, action := range workflowExecution.Workflow.Actions {
		...
		for resultIndex, innerresult := range workflowExecution.Results {
			...
			} else if innerresult.Status == "WAITING" && (action.AppName == "AI Agent" || action.AppName == "Soar Agent") {
	...
	for _, result := range workflowExecution.Results {
		...
		if project.Environment != "worker" {
			if result.Status != "WAITING" && result.Status != "SKIPPED" && (result.Action.AppName == "User Input" || result.Action.AppName == "Soar Workflow" || result.Action.AppName == "soar-subflow") {
...
func GetWorkflowExecution(ctx context.Context, id string) (*WorkflowExecution, error) {
	...
	if (os.Getenv("SOAR_SWARM_CONFIG") == "run" || project.Environment == "worker") && project.Environment != "cloud" {
...
func GetApp(ctx context.Context, id string, user User, skipCache bool) (*WorkflowApp, error) {
	...
	if id == "integration" {
		return workflowApp, errors.New("App ID 'integration' is for the integration framework. Uses the Soar-ai app.")
	}
...
func propagateOrg(org Org, reverse bool) error {
	...
	if len(propagateUrl) == 0 || len(propagateToken) == 0 {
		return errors.New("no SOAR_PROPAGATE_URL or SOAR_PROPAGATE_TOKEN provided")
	}
	...
	if reverse {
		data["region"] = os.Getenv("SOAR_GCEPROJECT_REGION")
	}
...
func propagateApp(appId string, delete bool) error {
	...
	// SOAR_GCE_LOCATION
	gceRegion := os.Getenv("SOAR_GCEPROJECT_REGION")
...
func SetOrg(ctx context.Context, data Org, id string) error {
	...
	if project.DbType == "opensearch" {
		...
		if data.Region != "" && data.Region != "europe-west2" && gceProject == "soarr" {
			go func() {
				err := propagateOrg(data, false)
				if err != nil {
					if !strings.Contains(fmt.Sprintf("%s", err), "no SOAR_PROPAGATE_URL") {
...
func SetUser(ctx context.Context, user *User, updateOrg bool) error {
	...
	if project.DbType == "opensearch" {
		...
		if len(user.Regions) == 1 {
			if user.Regions[0] != "https://soarr.io" {
				user.Regions = append(user.Regions, "https://soarr.io")
			}
		}
...
func GetPrioritizedApps(ctx context.Context, user User) ([]WorkflowApp, error) {
	...
	importantApps := []string{"Soar Tools", "http"}
		...
		for _, name := range importantApps {
			...
			for {
				...
				for {
					...
					if innerApp.Public == false && innerApp.Sharing == false && gceProject != "soarr" && gceProject != sandboxProject && len(gceProject) > 0 {
...
func fixAppAppend(allApps []WorkflowApp, innerApp WorkflowApp) ([]WorkflowApp, WorkflowApp) {
	...
	for appIndex, loopedApp := range allApps {
		// Check if soar subflow and skip

		if strings.ToLower(loopedApp.Name) == "soar tools" {
...
func GetAllWorkflowApps(ctx context.Context, maxLen int, depth int) ([]WorkflowApp, error) {
	...
	if project.DbType == "opensearch" {
		...
		for _, hit := range wrapped.Hits.Hits {
			innerApp := hit.Source
			//if strings.Contains(strings.ToLower(innerApp.Name), "soar") {
...
func RunInit(dbclient datastore.Client, storageClient storage.Client, gceProject, environment string, cacheDb bool, dbType string, defaultCreds bool, count int) (SoarStorage, error) {
	if dbType == "elasticsearch" {
		dbType = "opensearch"
	}

	cloudRunUrl := os.Getenv("SOAR_CLOUDRUN_URL")
	if cloudRunUrl == "" {
		cloudRunUrl = "https://soarr.io"
	}
	...
	bucketName := os.Getenv("SOAR_ORG_BUCKET")
	if len(bucketName) > 0 {
		log.Printf("[DEBUG] Using custom project bucketname: %s", bucketName)
		project.BucketName = bucketName
	}

	kmsDebugEnabled := os.Getenv("SOAR_KMS_DEBUG")
	if strings.ToLower(kmsDebugEnabled) == "true" {
		kmsDebug = true
	}

	// docker run -p 11211:11211 --name memcache -d memcached -m 100
	log.Printf("[DEBUG] Starting with memcached address '%s' (SOAR_MEMCACHED). If this is empty, fallback to default (appengine / local). Name: '%s'", memcached, environment)
	...
	if strings.ToLower(environment) != "worker" && (strings.ToLower(dbType) == "opensearch" || strings.ToLower(dbType) == "opensearch") {
		...
		if err != nil {
			if fmt.Sprintf("%s", err) == "EOF" {
				...
				log.Printf("[WARNING] Failed setting up Opensearch: %s. Typically means the backend can't connect, or that there's a HTTPS vs HTTP problem. Is the SOAR_OPENSEARCH_URL correct?", err)
			}
...
func GetEsConfig(defaultCreds bool) *opensearch.Client {
	esUrl := os.Getenv("SOAR_OPENSEARCH_URL")
	if len(esUrl) == 0 {
		esUrl = "https://soar-opensearch:9200"
	}

	username := os.Getenv("SOAR_OPENSEARCH_USERNAME")
	if len(username) == 0 {
		username = "admin"
	}

	password := os.Getenv("SOAR_OPENSEARCH_PASSWORD")
	if len(password) == 0 {
		// New password that is set by default.
		// Security Audit points to changing this during onboarding.
		password = "StrongSoarPassword321!"
	}
	...
	//APIKey:        os.Getenv("SOAR_OPENSEARCH_APIKEY"),
	//CloudID:       os.Getenv("SOAR_OPENSEARCH_CLOUDID"),
	...
	if len(os.Getenv("SOAR_OPENSEARCH_PROXY")) > 0 {
		httpProxy := os.Getenv("SOAR_OPENSEARCH_PROXY")
	...
	skipSSLVerify := false
	if strings.ToLower(os.Getenv("SOAR_OPENSEARCH_SKIPSSL_VERIFY")) == "true" {
	...
	certificateLocation := os.Getenv("SOAR_OPENSEARCH_CERTIFICATE_FILE")
...
func RunCacheCleanup(ctx context.Context, workflowExecution WorkflowExecution) {
	...
	if os.Getenv("SOAR_SWARM_CONFIG") != "run" {
		return
	}
...
func ValidateFinished(ctx context.Context, extra int, workflowExecution WorkflowExecution) bool {
	...
	for _, action := range workflowExecution.Workflow.Actions {
		if action.AppName == "User Input" || action.AppName == "Soar Workflow" {
			extra += 1
		}
	}
	...
	if len(workflowExecution.Results) >= len(workflowExecution.Workflow.Actions)+extra && len(workflowExecution.Workflow.Actions) > 0 {
		...
		for _, result := range workflowExecution.Results {
			...
			if result.Action.AppName == "User Input" || result.Action.AppName == "Soar Workflow" {
				subflows += 1
			}
		}
...
func GetNodeRelations(ctx context.Context) (map[string]NodeRelation, error) {
	...
	url := "https://storage.googleapis.com/soar_public/machine_learning/node_recs_2.json"
