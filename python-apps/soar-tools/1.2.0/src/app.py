...
class Tools(AppBase):
    __version__ = "1.2.0"
    app_name = (
        "Soar Tools"  # this needs to match "name" in api.yaml for WALKOFF to work
    )
    ...
    def send_email_soar(self, apikey, recipients, subject, body, attachments=""):
        ...
        try:
            ...
            try:
                ...
                if "'return' outside function" in str(e):
                    return {
                        "success": False,
                        "message": f"SyntaxError - Soar Recommendation: Instead of using 'return' without a function, use 'exit()' to return when not inside a function. Raw Syntax error: {e}",
                    }
                else:
                    return {
                        "success": False,
    ...
    def execute_bash(self, code, soar_input):
    ...
    def merge_lists(self, list_one, list_two, set_field="", sort_key_list_one="", sort_key_list_two=""):
        ...
        try:
            ...
            return {
                "success": False,
                "reason": "An error occurred while merging the lists. PS: List one can NOT be a list of integers. If this persists, contact us at support@soarr.io",
                "exception": f"{e}",
            }
