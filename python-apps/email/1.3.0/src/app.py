...
class Email(AppBase):
    ...
    # This is an email function of Soar
    def send_email_soar(self, apikey, recipients, subject, body):
        targets = [recipients]
        if ", " in recipients:
        ...
        url = "https://soarr.io/functions/sendmail"
    ...
    def send_email_smtp(self, smtp_host, recipient, subject, body, smtp_port, attachments="", username="", password="", ssl_verify="True", body_type="html", cc_emails=""):
        ...
        if "office365.com" in smtp_host:
            return {
                "success": False,
                "reason": "Office 365 does not easily support SMTP anymore, and recommends the Graph API. Please use the Outlook Office365 app in Soar with the 'send email' action."
            }
    ...
    def get_emails_imap(
        ...
        upload_email_soar = (
            True if str(upload_email_soar).lower().strip() == "true" else False
        )
        upload_attachments_soar = (
            True if str(upload_attachments_soar).lower().strip() == "true" else False
        )
        ...
        try:
            ...
            for i in range(len(id_list) - 1, len(id_list) - amount - 1, -1):
                ...
                if upload_email_soar:
                    self.logger.info("Uploading email to soar")
                    email_up = [{"filename": "email.msg", "data": data[0][1]}]
                    email_id = self.set_files(email_up)
                    output_dict["email_uid"] = email_id[0]

                if upload_attachments_soar:
                    self.logger.info("Uploading email ATTACHMENTS to soar")
                    try:
                        atts_up = [{
                                "filename": x["filename"],
    ...
    def parse_email_file(self, file_id, extract_attachments=False):
        ...
        try:
            ...
            # Put attachments in the soar file system
    ...
    # This is an SMS function of Soar
    def send_sms_soar(self, apikey, phone_numbers, body):
        phone_numbers = phone_numbers.replace(" ", "")
        targets = phone_numbers.split(",")

        data = {"numbers": targets, "body": body}

        url = "https://soarr.io/api/v1/functions/sendsms"
        headers = {"Authorization": "Bearer %s" % apikey}
